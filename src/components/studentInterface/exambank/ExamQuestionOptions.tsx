...
export default function ExamQuestionOptions({ question, index, total, setNumber }: ExamQuestionProps)
{
    ...
    const displayedOptions = question.options.map((option, index) => (
        <Box
            boxShadow={selectedOption === index ? check ? question.correctOption === index.toString() ? '0px 0px 0px 2px rgba(0,195,66,1)' : '0px 0px 0px 2px rgba(255,0,0,1)' : '0px 0px 0px 5px rgba(255,126,0,1)' : '0px 0px 0px 1px rgba(0,0,0,1)'}
            borderRadius='10px'
            sx={{
                textIndent: '25px',
                cursor: 'pointer'
        ...
        >
            ...
            {
                (selectedOption === index && check) ?
                question.correctOption === index.toString() ?
    ...
    return (
        ...
        >
            ...
            {
                check && selectedOption.toString() !== question.correctOption &&
                <Stack
                    bgcolor='#FEF4EB'
                    borderRadius='20px'
                    width='740px'
