...
export default function ExamQuestionOptions({ question, index, total, setNumber }: ExamQuestionProps)
{
    ...
    const displayedOptions = question.options.map((option, index) => (
        <Box
            boxShadow={selectedOption.includes(index.toString()) ? check ? question.correctOption.includes(index.toString()) ? '0px 0px 0px 2px rgba(0,195,66,1)' : '0px 0px 0px 2px rgba(255,0,0,1)' : '0px 0px 0px 5px rgba(255,126,0,1)' : '0px 0px 0px 1px rgba(0,0,0,1)'}
            borderRadius='10px'
            sx={{
                textIndent: '25px',
                cursor: 'pointer'
            ...
            width='790px'
            py={2}
            onClick={() => setSelectedOption(prev => prev.includes(index.toString()) ? prev.filter(item => item !== index.toString()) : prev.length > 2 ? [prev[0], prev[1], index.toString()] : [...prev, index.toString()])}
            position='relative'
        >
            <Typography>{option}</Typography>
            {
                (selectedOption.includes(index.toString()) && check) ?
    ...
    return (
        ...
        >
            ...
            {
                check && !(question.correctOption.every(answer => selectedOption.includes(answer))) &&
                <Stack
                    bgcolor='#FEF4EB'
                    borderRadius='20px'
                    width='740px'
