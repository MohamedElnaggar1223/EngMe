...
import { Button, Grid, Input, InputLabel, Stack, SvgIcon, Typography } from "@mui/material"
import { useState } from "react"
import { setKnowledgeBankContent } from "../../helpers/setKnowledgeBankContent"
...
export default function KnowledgeBankContent({ id }: KnowledgeBankContent) 
{
    ...
    const { mutate } = useMutation({
        onMutate: () => {
            const previousData = queryClient.getQueryData(['knowledgeBankContent', id])

            queryClient.setQueryData(['knowledgeBankContent', id], (oldData: unknown) => {
                //@ts-expect-error oldData
                return (oldData && oldData.length > 0) ? [...oldData, { title, type: 'KnowledgeBank/', majorId: id, content: file?.name }] : [{ title, type: 'KnowledgeBank/', majorId: id, content: file?.name }]
            })
    ...
    const { mutate: mutateDelete } = useMutation({
        onMutate: (kbContentId: string) => {
            const previousData = queryClient.getQueryData(['knowledgeBankContent', id])

            queryClient.setQueryData(['knowledgeBankContent', id], (oldData: unknown) => {
                //@ts-expect-error oldData
                const newData = (oldData && oldData.length > 0) ? oldData.slice().filter(data => data.id !== kbContentId) : []
                return newData
            })
    ...
    const displayedContent = knowledgeBankContent?.map(kbContent => (
        <Grid item xs={8} key={kbContent.id}>
            <Stack
                direction='column'
                position='relative'
                width='auto'
        ...
        >
            ...
            >
                ...
                >
                    ...
                    <Typography textAlign='left' noWrap fontFamily='Inter' fontSize={14}>Add Pdf</Typography>
                </Button>
            </Stack>
            {/* <Stack
                flexWrap='wrap'
